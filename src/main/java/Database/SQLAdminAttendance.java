...
public class SQLAdminAttendance {
    public ObservableList<Attendance> getAttendance(int id) {
        ObservableList<Attendance> attendanceList = FXCollections.observableArrayList();
        String command = "SELECT * FROM tbl_attendance_admin WHERE emp_id = ?";
    ...
    public ObservableList<Attendance> getDailyAttendance(Date dt1, Date dt2) {
        ...
        String command = "SELECT attendance_id, " +
                ...
                "tbl_attendance_admin.emp_attendance_date, " +
                "tbl_attendance_admin.emp_timein, " +
                "tbl_attendance_admin.emp_timeout " +

                "FROM tbl_attendance_admin " +
                "JOIN tbl_employees " +
                "ON tbl_attendance_admin.emp_id = tbl_employees.emp_id " +
                "JOIN tbl_department " +
    ...
    public void registerAttendance(Attendance attendance) {
        String command = "INSERT INTO tbl_attendance_admin (emp_id, emp_attendance_date, emp_timein, emp_timeout) " +
                "VALUES (?, ?, ?, ?)";
        try (Connection connection = connect();
             PreparedStatement preparedStatement = connection.prepareStatement(command)) {
...
//                        resultSet.getInt("late_hours"),
    ...
    public void deleteAttendance(Attendance attendance) {
        String command = "DELETE FROM tbl_attendance_admin " +
                "WHERE attendance_id = ?";
        try (Connection connection = connect();
             PreparedStatement preparedStatement = connection.prepareStatement(command)) {
    ...
    public void updateAttendance(Attendance attendance) {
        String command = "UPDATE tbl_attendance_admin " +
                "SET emp_timein = ?, " +
                "emp_timeout = ?, " +
                "emp_attendance_date = ? " +
                "WHERE attendance_id = ?";
    ...
    public void registerAttendanceUsingQR(Attendance attd) {
        boolean exist = !checkIfAttendanceExist(attd);
        ...
        if (exist) {
            ...
            try (Connection connection = connect();
                ...
                preparedStatement.setTime(1, attd.getEmployee_TimeOut()); // Need to connect to dept table
                preparedStatement.setDate(2, attd.getEmployee_Attendance_Date()); //
                preparedStatement.setInt(3, attd.getEmployee_ID());
...
//                callAlert("Attendance has been updated!", 2);
            ...
            try (Connection connection = connect();
                ...
                preparedStatement.setInt(1, attd.getEmployee_ID());
                preparedStatement.setDate(2, attd.getEmployee_Attendance_Date()); //
                preparedStatement.setTime(3, attd.getEmployee_TimeIn());
                preparedStatement.setTime(4, attd.getEmployee_TimeIn());
